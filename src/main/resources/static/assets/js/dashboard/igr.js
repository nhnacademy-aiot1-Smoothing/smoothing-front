async function fetchData() {
    try {
        ...
        const class_resp = await fetch("/sensor/igr/class");
        const office_resp = await fetch("/sensor/igr/office");

        const class_data = await class_resp.json();
        const office_data = await office_resp.json();

        const class_igr = parseInt(class_data.value);
        const office_igr = parseInt(office_data.value);

        let normal;
        let abnormal;

        if (class_igr===0 && office_igr===0) {
            normal = 2000;
            abnormal = 0;
        } else {
            normal = 0;
